...
class GraphGather(nn.Module):
    ...
    def forward(self, hidden_nodes, input_nodes, node_mask):
        #print('gatherforward')
        #torch.cat是一个拼接函数,在dim维上拼接
        #将原始特征和经过mpnn更新的特征拼接到一起
...
class Side_Encoder(nn.Module):
    def __init__(self):
        super(Side_Encoder, self).__init__()
        self.sider_encoder=torch.nn.Embedding(663,90)
    def forward(self, input):
        #print(input)
        batch=input.size(0)
        #print('batch',batch)
        a=torch.LongTensor(1,1)
        input=input.type_as(a)

        side_fts=self.sider_encoder(input).view(batch,-1)
        #print(side_fts.shape)
        #print(input)
        return side_fts

class Protein_Encoder(nn.Module):
    def __init__(self,node_features=1033, emb_features=[256],out_features=45,emb_dropout_p=0.0):
        super(Protein_Encoder, self).__init__()
        self.protein_encoder=FeedForwardNetwork(node_features, emb_features, out_features, dropout_p=emb_dropout_p, bias=False)
    def forward(self, input):

        protein_fts=self.protein_encoder(input)
        return protein_fts

class Momo_Encoder(nn.Module):
    def __init__(self,node_features=9262, emb_features=[256],out_features=45,emb_dropout_p=0.0):
        super(Momo_Encoder, self).__init__()
        self.momo_encoder=FeedForwardNetwork(node_features, emb_features, out_features, dropout_p=emb_dropout_p, bias=False)
    def forward(self, input):
        momo_fts=self.momo_encoder(input)
        return momo_fts

class Structure_Encoder(nn.Module):
    def __init__(self,node_features=512, emb_features=[256],out_features=45,emb_dropout_p=0.0):
        super(Structure_Encoder, self).__init__()
        self.stru_encoder=FeedForwardNetwork(node_features, emb_features, out_features, dropout_p=emb_dropout_p, bias=False)
    def forward(self, input):
        stru_fts=self.stru_encoder(input)
        return stru_fts
...
class FeedForwardNetwork(nn.Module):
    ...
    def __init__(self, in_features, hidden_layer_sizes, out_features, activation='SELU', bias=False, dropout_p=0.0):
        ...
        if activation == 'SELU':
            ...
            Dropout = nn.Dropout
            init_constant = 2.0
        #hidden_layer_sizes =[msg_hidden_dim] * msg_depth  msg_depth层隐藏向量的总维度
        ...
        #模块将按照它们在构造函数中被传递的顺序添加到它;或者，也可以传入一个有序的模块字典。
        #将这些网络层连接起来
